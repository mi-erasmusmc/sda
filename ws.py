...
service_url = 'http://aneurist.erasmusmc.nl/PeregrineWebservice/PeregrineWebserviceImplPort'
...
def getConceptName(id):
    XML = client.getConcept(arg0 = id)
    try:
    	return XML.name
    except:
	return ""

def getSemanticTypes(conceptId):
    XML = client.getSemanticTypes(arg0 = conceptId)
    try:
    	semanticGroup = xml_str_encoder(semanticGroupTypeDict[int(XML.id)]['group'])
        semanticTypeName = xml_str_encoder(semanticGroupTypeDict[int(XML.id)]['name'])
    except:
        semanticGroup = "unknown"
        semanticTypeName = "unknown"
    return "<semantictypes><semantictype id=\"" + XML.id + "\" group=\"" + semanticGroup + "\">" + semanticTypeName + "</semantictype></semantictypes>" 
...
def outputConcept(concept):
    ...
    res += "<name>" + xml_str_encoder(getConceptName(concept.conceptId)) + "</name>"
    res += getSemanticTypes(concept.conceptId)
...
def indexText(text):
    client.index( arg0 = text )
    XML       = client.XML( arg0 = text )
